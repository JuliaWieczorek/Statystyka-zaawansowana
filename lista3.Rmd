...
test_n <- wilcox.test(r2017, dane)$statistic #wg dokumentacji tzn 2-gi wieksze od 1-go argumentu
cnt <- 0 # licznik, zwiększany indykator
z <- c(dane,r2017) #oba zbiory w jeden wektor
combin <- combn((m+n),m) #macierz wszystkich kombinacji m-elementowych ze zbioru m+n-elementowego
for (i in 2:N){ #od i=2, bo 
  n_przed1 <- z[combin[,i]] #nowy wektor przed
  n_po1 <- z[-combin[,i]] #nowy wektor po
  n_test1 <- wilcox.test(n_przed1,n_po1)$statistic
  if(n_test1 <= test_n){cnt=cnt+1} #wzrost indykatora, gdy prawdziwa nierówność
}
c <- cnt/N
